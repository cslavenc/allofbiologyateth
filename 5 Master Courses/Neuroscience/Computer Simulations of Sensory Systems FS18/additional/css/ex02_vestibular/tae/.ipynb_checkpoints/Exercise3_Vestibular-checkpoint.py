...
def Calc_true_align(true_align, data, sensor_alignment):
    ...
    # true align [0, 9.81, 0] => das wären die daten, wenn der Sensor flach auf dem Tisch liegen würde
    # gyro data = rotationen
    # sensor alignment = initial alignment => das sind die Accelerationen, die gemessen werden bei t=0
    ...
    #print('degree in deg: {}'.format(np.rad2deg(degree_rad)))
    ...
    rotating_90degrees=k.rotmat.R(axis=0, angle=90)
...
def Calc_canal_stimulation(new_data, counter):
    ...
    #calculating the stimulaside amplitude
    ...
    Canals = {'info': 'The matrix rows describe horizontal, anterior, and posterior canal orientation',
     ...
     'left': np.array(
            [[-0.365,  0.158,  0.905],
             [-0.652,  0.753,  0.017],        CaCanals[side] = (Canals[side].T / np.sqrt(np.sum(Canals[side]**2, axis=1))).T for side in ['right', 'left']:
        Canals[side] = (Canade].T / np.sqrt(np.sum(Canals[side]**2, axis=1))).T
    
    #number of entries
    list_n
    print(stim_right[0])um=new_data.shape[0]
...
def Nose_orientation(new_data):
    ...
    rotation_head_tilt=k.rotmat.R(axis=1, angle=15)
    ...
    nose_rotmat=k.quat.convert(orientation[-1])
...
if __name__ == "__main__":
    ...
    data = XSens('Walking_02.txt', q_type=None)
    #data=getXsens.getXSensData('Walking_02.txrot', ['Counter', 'Acc', 'Gyr'])
    
    #get counter column
    counter=np.arange(2552,3504)

    #get the gyroscope data
    gyro_data=data.omega
    
    #get the accelerometer data
    xyz_acceleration=data.acc

    #sensor alignment
    # this is the sensor at t=0
