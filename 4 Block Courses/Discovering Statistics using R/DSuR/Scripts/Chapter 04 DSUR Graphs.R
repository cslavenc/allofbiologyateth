...
############################# BIO 209 Disclaimer ###############################
## Since the book was published, the ggplot2 package has been updated a couple##
## of times, which unfortunately means that some of the arguments in certain  ##
## functions have changed slightly. I have updated the script below, and it   ##
## all should work fine now.                                                  ##
## Unfortunately, this means that some of the code in the book is not 100%    ##
## correct anymore, so stick to the example codes in this script and the      ##
## lecture slides! I have been in touch with Andy Field about this, and in a  ##
## new edition of the book the outdated code will be replaced: an unfortunate ##
## example of R being too dynamic for its own good sometimes!                 ##
################################################################################
...
scatter + geom_point() +
          geom_smooth() +
          labs(x = "Exam Anxiety", y = "Exam Performance %")
## R generates a warning message, but this is no error!
...
festivalDensity + geom_histogram(binwidth= 0.4) +
		              labs(x= "Hygiene (Day 1 of Festival)", y= "Frequency") +
...
## Again, R generates a warning message, this is not an error!
...
## I also updated RSoul's tip 4.3, page 155
...
## Another change to RSoul's tip 4.3, page 155
bar <- ggplot(chickFlick, aes(film, arousal, fill= gender))
bar + stat_summary(fun.y= mean, geom= "bar", position="dodge") +
		  stat_summary(fun.data= mean_cl_normal, geom= "errorbar", position= position_dodge(width= 0.90), width= 0.2) +
		  labs(x= "Film", y= "Mean Arousal", fill= "Gender") +
...
graph + geom_smooth(aes(colour= Rating_Type), method= lm) +
		    labs(title= "geom_smooth(aes(colour= Rating_Type), method= lm)")
# saveInImageDirectory("04 Tutorial colour lm.png")

graph + geom_smooth(aes(colour= Rating_Type), method= lm, se= F) +
		    labs(title= "geom_smooth(aes(colour= Rating_Type), method= lm, se= F)")
...
LecturerData$job<- factor(LecturerData$job, label= c("Lecturer", "Student"))
...
line + stat_summary(fun.y= mean, geom= "point") +
		   stat_summary(fun.y= mean, geom= "line", aes(group= 1), colour= "Red", linetype= "dashed") +
		   stat_summary(fun.data= mean_cl_normal, geom= "errorbar", width= 0.2) +
		   labs(x= "Group", y= "Mean Neuroticism")
...
scatter + geom_point() +
		      geom_smooth(method= "lm", aes(fill= job), alpha= 0.1) +
		      labs(x= "Neuroticism", y= "Alcohol Consumption")

#-------------------------Smart Alex Task 2

	Infidelity<- read.csv("Infidelity Data.csv")
...
Infidelity$id<- row(Infidelity[1])
Bullets<- reshape(Infidelity, idvar = c("id", "Gender"), varying = c("Partner", "Self"), v.names = "Number_of_Bullets", timevar = "Recipient", times = c(0:1), direction = "long")
