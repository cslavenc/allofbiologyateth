...
histogram<-ggplot(album2, aes(studentized.residuals)) + geom_histogram(aes(y=..density..), colour="black", fill="white") + labs(x = "Studentized Residual", y = "Density")
...
Supermodel<-read.table("Supermodel.dat", sep= " ", header = TRUE)
names(Supermodel)<- c("salary", "age", "years", "beauty")
...
#-----Read in data for Glastonbury Festival Regression----
gfr<-read.table(file="GlastonburyFestivalRegression.dat", header=TRUE, as.is=TRUE)
...
gfr.1 <- lm(gfr$change ~ gfr$crusty + gfr$metaller + gfr$indie.kid, data=gfr, na.action=na.exclude)
...
ChildAggression<-read.table("ChildAggression.dat", sep= " ", header = TRUE)
