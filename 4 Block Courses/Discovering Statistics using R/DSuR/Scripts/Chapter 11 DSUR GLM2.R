...
#Robust Tests
noCloak<-subset(invisibilityData, cloak=="Cloak",)
invisCloak<-subset(invisibilityData, cloak=="No Cloak",)
...
stalkerModel<-aov(stalk2~stalk1 + group, data = stalkerData)
Anova(stalkerModel, type = "III")

#adjusted means:
adjustedMeans<-effect("group", stalkerModel, se = TRUE)
summary(adjustedMeans)
adjustedMeans$se

#Regression parameter for the covariate:
summary.lm(stalkerModel)

#Plots:
plot(stalkerModel)

#Check for homogenity of regression slopes:
hoRS<-update(stalkerModel, .~. + stalk1:group)
Anova(hoRS, type = "III")
