...
#Set Phallus as a factor:

GallupData$Phallus<-factor(GallupData$Phallus, levels = c(1:3), labels = c("No Coronal Ridge", "Minimal Coronal Ridge", "Coronal Ridge"))
#Error bar graph
...
#Again we need Phallus to be set at a factor, (because you have already set Phallus to be a factor above, you don't really need to do this again):

Phallus<-factor(Phallus, levels = c(1:3), labels = c("No Coronal Ridge", "Minimal Coronal Ridge", "Coronal Ridge"))
...
#Set the categorical variable 'music' to be a factor:

festivalData$music<-factor(festivalData$music, levels = c(1:4), labels = c("Indie kid", "Metaller", "Crusty", "No Musical Affiliation"))
