...
# not working likely
# PRE : model must be based on E.coli
# POST: returns names and ids of alternative carbon sources
def Csources_ecoli(model):
    Csources = []
    Csources_names = []
    
    for x in model.metabolites:
        if 'C' in x.formula[0] and 'e' in x.compartment:
            Csources.append(x.id)
            Csources_names.append(x.name)
            
    return Csources, Csources_names
