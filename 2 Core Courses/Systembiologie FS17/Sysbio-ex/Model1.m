...
parameterValues = 0.16:0.1:5;
for i=1:length(parameterValues)
    % set the new value of the parameter
    set(parameter6,'Value',parameterValues(i));
    % simulate model with altered parameter, simData contain all the data
    simData = sbiosimulate(modelObj1);
    % plot data
    ...
    %------------- ADD CODE HERE ------------%
    % you can use "hold on" to hold the current plot and add another to the same figure
    % you can use "hold off" to terminate this functionality

end

parameterValues2 = 5:1:20;
for j=1:length(parameterValues2)
    set(parameter7, 'Value', parameterValues2(j));
    simData = sbiosimulate(modelObj1);
...
parameter6 = sbioselect(modelObj1,'Type','parameter','Where','Name','==','KmPFK');
set(parameter6,'Value', 0.16);
parameter7 = sbioselect(modelObj1,'Type','parameter','Where','Name','==','VmaxPYK');
set(parameter7,'Value', 5);
